...
def main():
    ...
    print("Chat with the colleague directory assistant (with streaming PII filter). Type 'exit' to quit.\n")
    ...
    while True:
        ...
        full_filtered_response = ""
        for chunk in client.stream(messages):
            chunk_text = chunk.content
            if chunk_text:
                filtered = guardrail.process_chunk(chunk_text)
                if filtered:
                    print(filtered, end="", flush=True)
                    full_filtered_response += filtered

        # Flush any remaining content in the buffer
        remaining = guardrail.finalize()
        if remaining:
            print(remaining, end="", flush=True)
            full_filtered_response += remaining

        print("\n")

        messages.append(AIMessage(content=full_filtered_response))
